...
select name, count(sf.film_id) as "Films number"  
from sakila.film sf
left join sakila.film_category sfc 
on sf.film_id = sfc.film_id
left join sakila.category sc 
...
select -- p.*, c.store_id as customer_store, s.store_id as staff_store_id
sum(case when c.store_id > s.store_id then amount end) as c2s1,
sum(case when c.store_id < s.store_id then amount end) as c1s2
from payment p
...
select avg_runningtimes.name, round(max(avg_runningtime),2) as max_avg_runningtime
from avg_runningtimes
group by avg_runningtimes.name
order by max_avg_runningtime desc;
...
select count(*) > 0 as can_rent
from sakila.film f
join sakila.inventory i 
on f.film_id = i.film_id 
join sakila.store s
on i.store_id = s.store_id 
where f.title = "Academy Dinosaur" and s.store_id = 2;
